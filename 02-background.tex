...
An LLM is first pre-trained by maximum-likelihood next-token
...
The RLHF pipeline of InstructGPT chains the two: SFT, then
...
the policy close to the SFT reference~\cite{ziegler2019finetuning,stiennon2020summarize}. Notably, a 1.3B-parameter model aligned this way was preferred by
humans over the 175B GPT-3~\cite{ouyang2022instructgpt}, establishing
that alignment via RL can be more effective than scaling parameters
alone.

\subsection{A Unified MDP/POMDP View}

Following~\cite{zhang2026landscape}, we cast both
...
$\mathcal{A}_{\text{agent}}=\mathcal{A}_{\text{text}}\cup\mathcal{A}_{\text{action}}$, where actions invoke tools or modify the environment. Rewards become
step-wise, mixing task completion with dense sub-rewards, and the
...
Four algorithm families underlie most methods in this survey:
...
$A(s_t,a_t)=R(s_t,a_t)-V(s_t)$. PPO became the standart optimizer for RLHF but
requires a critic of comparable size to the policy~\cite{schulman2017ppo}.

\paragraph{DPO}
removes the reward model, reformulating the
...
as an implicit reward model~\cite{rafailov2023dpo}.

\paragraph{GRPO}
removes the critic: it samples a group of responses per prompt and
normalizes their rewards to form advantages~\cite{shao2024deepseekmath}. Combined
